...
namespace Kengic.Was.Connector.WebEasy
{
    public class WebEasySocketClient : IConnector
    {
        private readonly MessageHelper _JdMessage2 = new MessageHelper();
        private byte[] _beforeBytes = {};
        private string _logName;
        private SocketsClientThread _socketsClientThread;

        public WebEasySocketClient(ConnectorElement connectorElement)
        {
        ...
        private void Client_OnReceiveMessage(byte[] infoBytes)
        {
            try
            {
                ...
                _JdMessage2.GetReceiveMessageForClient(messageBytes, ref messageDict, ref _beforeBytes);
                //查看一下接收的数据到底是什么
                ...
                foreach (var theKeys in messageDict.Keys)
                {
                   ...
                   // var messageArray = messageDict[theKeys].Split(':');
                    ...
                    if (messageArray[2] == "PING")
                    {
                        ...
                        //var akMessage = ":" + messageArray[0] + ":ACK";
