...
namespace Kengic.Was.Connector.Vip
{
    public class VipClient : IConnector
    {
        private readonly VipMessage _vipMessage = new VipMessage();
        private byte[] _beforeBytes = {};
        private string _logName;
        private SocketsClientThread _socketsClientThread;

        public VipClient(ConnectorElement connectorElement)
        {
        ...
        private void Client_OnReceiveMessage(byte[] infoBytes)
        {
            try
            {
                ...
                _vipMessage.GetReceiveMessageForClient(vipMessageBytes, ref messageDict, ref _beforeBytes);
                ...
                foreach (var theKeys in messageDict.Keys)
                {
                    ...
                    {
                        SendMessage(messageDict[theKeys]);
                        ConnectStatus = true;
