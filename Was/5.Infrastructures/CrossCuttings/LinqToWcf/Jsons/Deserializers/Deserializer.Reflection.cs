...
namespace Kengic.Was.CrossCutting.LinqToWcf.Jsons
{
    partial class Deserializer
    {
        ...
        private Type Type(JToken token)
        {
            ...
            var assemblies = new Dictionary<string, Dictionary<string, Type>>();
            ...
            {
                if (assemblyName.Contains("http://schemas.datacontract.org/2004/07/"))
                {
                    var dataTypeResovler = DataContractResolverExtensions.KnownTypeDataContractResolver;
                    type = dataTypeResovler.ResolveName(typeName, assemblyName, null, dataTypeResovler);
                }
                if (assemblyName == "AnonymousType")
                {
                    type = typeof (object);
                }
                else
        ...
        private MemberInfo Member(JToken token)
        {
            ...
            return members.First(p => p.MemberType == memberType && p.Name == name);
        }
    }
}