...
namespace Kengic.Was.CrossCutting.LinqToWcf
{
    ...
    public class InterLinqQuery<T> : InterLinqQueryBase, IOrderedQueryable<T>
    {
        ...
        public InterLinqQuery(IQueryProvider iQueryProvider, Expression expr, object additionalObject, string queryName,
        ...
        {
            var retrievedObjects = (IEnumerable) provider.Execute(expression);
            ...
            if (!string.IsNullOrEmpty(queryName))
                QueryParameters = (parameters != null) && (parameters.Count() != 0)
                    ? new List<SerializableExpression>(
                        parameters.Select(s => Expression.Constant(s).MakeSerializable())
                            .ToArray())
                    : null;
            else
