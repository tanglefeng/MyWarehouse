...
namespace Kengic.Was.CrossCutting.LinqToWcf
{
    ...
    public abstract class InterLinqContext
    {
        ...
        public IQueryable<T> ExecuteMethod<T>(object additionalObject, string name, params object[] parameters)
            where T : class
        {
            return QueryHandler.Get<T>(additionalObject, name, null, parameters);
        }

        public IQueryable ExecuteMethod(Type type, object additionalObject, string name, params object[] parameters)
        {
            return QueryHandler.Get(type, additionalObject, name, null, parameters);
        }
    }
}